...
mod_map_confirmed_diphtheria_cases_penta1_coverage_annual_data_ui <- function(id){
  ...
  tagList(
    ...
    div(class = "col-12 col-12-t measles-col",
        ...
        span(class = "info-icon-container",
             tags$a(class = "info-icon-link", href="#",
                    ...
                    span(class="info-tooltiptext",
                         p(style="color:#ffffff;font-size:10px;algin:left;", "The blue bubbles represent clusters of Diphtheria cases in a State. The numbers in each bubble are cases in that cluster"),
        ...
        HTML("<h6 class = 'column-title column-title-map'>Chart 7: Confirmed diphtheria cases, Penta 1 coverage </h6>"),
        ...
        screenshotButton(id = ns("penta_map"), filename = ">Chart 7 Confirmed diphtheria cases Penta 1 coverage", download =T, scale = 2, label = "", class = "download-data-btn download-chart-btn"),

        withSpinner(leafletOutput(ns("penta_map"), height=440),type = 6, size = 0.4,hide.ui = F)
    )
...
mod_map_confirmed_diphtheria_cases_penta1_coverage_annual_data_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    observe({
      ...
      if(sum(picker_state_var() == "Federal Government") == 1){
        ...
        if(sum(picker_month_var() == "Year Data") == 1){
          ...
          stream2_data$dhis2_data <- dplyr::tbl(connection, "penta_coverage_map") %>%
      ...
      if(sum(picker_state_var() == "Federal Government") == 1){
        ...
        if(sum(picker_month_var() == "Year Data") == 1){
          ...
          stream2_data$sormas_mvc <- dplyr::tbl(connection, "diphtheria_cases_map") %>%
    ...
    output$penta_map <-  renderLeaflet({mvc_map_leaflet()})
    ...
    output$downloadData <- downloadHandler(
      ...
      filename = function() {
        paste0("Chart 7- Penta",  picker_state_var(), picker_year_var() , picker_month_var() ,".zip")
      },
      content = function(fname) {

        write.csv(stream2_data$dhis2_data, file = "Penta coverage.csv", sep =",")

        write.csv(stream2_data$sormas_mvc, file = "sormas Penta cases.csv", sep =",")

        zip(zipfile=fname, files=c("Penta coverage.csv","sormas Penta cases.csv"))
      },
