...
mod_penta_vaccine_stock_analysis_penta_coverage_server <- function(id,
  ...
  moduleServer( id, function(input, output, session){
    ...
    chart_data <- reactive({
      ...
      dplyr::tbl(connection, "penta_stock_analysis")%>%
    ...
    # chart_data <-  dplyr::tbl(connection,"penta_stock_analysis") %>%
    ...
    indicator_plot <- reactive({
      ...
      plotM <- plotM %>% add_trace(x = ~Months,
                                   ...
                                   line = list(shape = 'spline', linetype = I("solid")),
                                   marker = list(symbol = I("circle")),
                                   name = 'Penta Doses Available (Opening Balance+Received)',
                                   yaxis = 'y2',
                                   hovertemplate = paste('<b>Number</b>: %{y:.0f}',
                                                         '<br><b style="text-align:left;">Month </b>: %{x}<br>'),
    ...
    output$downloadData <- downloadHandler(
      ...
      filename = function() {
        paste0("Chart 3- Diphtheria",  picker_state_var(), picker_lga_var() ,".csv")
      },
